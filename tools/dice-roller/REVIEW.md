...
Rolls between one and six dice from `Math.random()`, shows them as the Unicode
die faces ⚀–⚅, and prints the total. The count is clamped in the script as well
as constrained by the input's `min` and `max`, so typing past the range or
clearing the box cannot produce a nonsense roll.
...
any policy the file declared for itself removed. 1342 bytes in, 1488 out.
